...
interface ClareamentoImageLayoutProps {
    images: ProcedureImages[];
}

export default function ClareamentoImageLayout ({images}: ClareamentoImageLayoutProps) {
    return (
        <div>
            {images.map((item, key) => (
                <div className="mb-6 md:mb-8" key={key}>
                    {item.title !== "" &&
                        <ScrollReveal direction="up" delay={0.1}>
                            <h3 className="text-base md:text-lg font-semibold text-gray-800 mb-4 md:mb-6 text-center">
                                {item.title}
                            </h3>
                        </ScrollReveal>
                    }
                                ...
                                {item.imgs.map((img, key2) => (
                                    <div key={key2} className="flex flex-col items-center">
                                        ...
                                        <div
                                            ...
                                            w-[70%] h-auto sm:w-[280px] sm:h-40 md:w-[340px] md:h-40 lg:w-[450px] lg:h-60 
                                            overflow-hidden"
                                        >
                                            <Image
